...
The pourpose of this library is to provide R users with a framework for Item Response Theory (IRT) models, allowing for the possibility to include regression effects for the person parameters.
...
1. raschreg provides a set of functions to analyise IRT models in the case of dochotomous items. As of today it is possible to fit one and tow parameter logistic models. Iin both cases it is possible to include explanatory variables of the ability parameter.
...
1. `n`: number of rows in the dataset.  
2. `J`: number of items (optional if you don't supply difficultiy and/or discrimination parameters).  
3. `delta`: a vector containing item difficulties.  
4. `alpha`: a vector containing item discriminations.  
5. `gamma`: a vector containing pseudo-guessing parameters.  
6. `reg`: a matrix containing explanatory variables of the ability parameter.  
7. `beta`: a vector regression coefficients.  
8. `ability`: a logical value that determines whether the ability parameters are returned or not.
